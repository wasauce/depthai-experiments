...
def create_pipeline():
    ...
    det_nn.setBlobPath(str(Path("models/frozen_inference_graph-june18-2021.blob").resolve().absolute()))
    ...
    veh_nn.setBlobPath(str(Path("models/vehicle-detection-adas-0002-2021.blob").resolve().absolute()))
    ...
    rec_nn.setBlobPath(str((Path(__file__).parent / Path('models/license-plate-recognition-barrier-0007-2021.blob')).resolve().absolute()))
    ...
    attr_nn.setBlobPath(str((Path(__file__).parent / Path('models/vehicle-attributes-recognition-barrier-0039-2021.blob')).resolve().absolute()))
...
def lic_thread(det_queue, det_pass, rec_queue):
    ...
    while running:
        try:
            ...
            eg_inpass_seq = in_pass.getSequenceNum()
            eg_inpass_seq -= 1
            orig_frame = frame_seq_map.get(eg_inpass_seq, None)
            print("lic_thread eg_inpass_seq: %r", eg_inpass_seq)
            print("lic_thread frame_seq_map: %r", frame_seq_map.keys())

            if orig_frame is None:
                print("lic_thread orig_frame is None")
                continue
            
            lic_last_seq = in_pass.getSequenceNum()
...
def veh_thread(det_queue, det_pass, attr_queue):
    ...
    while running:
        try:
            ...
            eg_inpass_seq -= 1
            orig_frame = frame_seq_map.get(eg_inpass_seq, None)
            print("veh_thread eg_inpass_seq: %r", eg_inpass_seq)
            print("veh_thread frame_seq_map: %r", frame_seq_map.keys())

            if orig_frame is None:
                print("veh_thread orig_frame is None")
                continue
...
def rec_thread(q_rec, q_pass):
    ...
    while running:
        ...
        try:
            ...
            print("rec_thread - rec_data: %r", rec_data)
            rec_frame = q_pass.get().getCvFrame()
        except RuntimeError as e:
            print("rec_thread - RuntimeError: %r", e)
            continue

        decoded_text = ""
        for idx in rec_data:
            print(" rec_threadidx: %r", idx)
            if idx == -1:
                break
...
with dai.Device(create_pipeline()) as device:
    ...
    try:
        while should_run():
            ...
            print("lic_last_seq: %r", lic_last_seq)
            print("veh_last_seq: %r", veh_last_seq)
